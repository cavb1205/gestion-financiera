...
export default function SelectStorePage() {
  ...
  useEffect(() => {
    ...
    const fetchStores = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/tiendas/list/tiendas/admin`, {
          method: 'GET',
          headers: {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white rounded-xl shadow-md p-6 max-w-md w-full text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-800 mb-2">Error al cargar tiendas</h2>
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-100 py-12 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800 text-center">Seleccione una tienda</h1>
            ...
            >
              <div className="p-6">
                <div className="flex items-start">
                  ...
                  <div>
                    <h3 className="text-2xl capitalize font-semibold text-gray-700">{store.tienda.nombre}</h3>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <p className={`font-medium ${store.tienda.caja >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      ${store.tienda.caja.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Inversión</p>
                    <p className="font-medium text-gray-500">${store.tienda.inversion.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Utilidades</p>
                    <p className="font-medium text-green-600">${store.tienda.utilidades.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Gastos</p>
                    <p className="font-medium text-red-600">${store.tienda.gastos.toLocaleString()}</p>
                  </div>
                </div>
              </div>
        ...
        {stores.length === 0 && !loading && (
          <div className="bg-white rounded-xl shadow-md p-8 text-center mt-8">
            ...
            <h2 className="text-xl font-semibold text-gray-800 mb-2">No tienes tiendas asignadas</h2>
            <p className="text-gray-600 mb-6">
              Parece que no tienes ninguna tienda asignada a tu cuenta. 
