...
export default function DashboardLayout({ children }) {
  ...
  const { user, profile, logout, selectedStore, isAuthenticated, loading } = useAuth();
 ...
 useEffect(() => {
  ...
  if (!isAuthenticated || !selectedStore) {
    // Ya deberíamos haber redirigido, pero por si acaso
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
      ...
      <div className="hidden md:flex md:w-64 bg-indigo-800 text-white flex-col">
        ...
        <nav className="flex-1 px-4 py-6">
          ...
          <Link 
            ...
            className={`flex items-center w-full p-3 rounded-lg mb-2 ${isActive('/dashboard/clientes') ? 'bg-indigo-700' : 'hover:bg-indigo-700'}`}
          >
          ...
          <Link 
            ...
            className={`flex items-center w-full p-3 rounded-lg mb-2 ${isActive('/dashboard/reportes') ? 'bg-indigo-700' : 'hover:bg-indigo-700'}`}
          >
          ...
          <Link 
            ...
            className={`flex items-center w-full p-3 rounded-lg ${isActive('/dashboard/configuracion') ? 'bg-indigo-700' : 'hover:bg-indigo-700'}`}
          >
          ...
          <div className="flex justify-between items-center px-6 py-4">
            <div className="flex items-center">
              <h1 className="text-xl font-semibold text-gray-800">
                {isActive('/dashboard') && !isActive('/dashboard/clientes') && 'Dashboard'}
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center">
                ...
                <div className="ml-2 text-right hidden sm:block">
                  <p className="text-gray-700 font-medium">{profile?.trabajador || 'Usuario'}</p>
